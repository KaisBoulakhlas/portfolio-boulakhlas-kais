...
const Navbar = () => {
    ...
    return (
        <nav onKeyDown={() => setToggle(!toggle)} onClick={() => setToggle(!toggle)} className={ scrolled ? "app__navbar scrolled" : "app__navbar" }>
                ...
                <ul>
                    {
                        menu?.map((item) => (
                            <li key={item}>
                                <LinkScroll activeClass="active" spy={true} to={item} href={`#${item}`} offset={-30} onClick={() => setToggle(false)}>{item}</LinkScroll>
                            </li>   
                        ))
                    }
                </ul>
